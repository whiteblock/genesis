...
func eos_createConf(servers []Server){
	...
	constantEntries := []string {
		...
		"p2p-listen-endpoint = localhost:9877",	"agent-name = \"EOS Test Agent\""}
...
func eos_getWallets(servers []Server) map[string]string {
	...
	for _,server := range servers {
		for _,ip := range server.ips {
			data := sshExec(server.addr,fmt.Sprintf("docker exec whiteblock-node%d cleos --wallet-url http://%s:8899 wallet create | tail -n 1",node,ip))
...
func eos_getKeyPairs(servers []Server) map[string]KeyPair {
	...
	for _,server := range servers {
		for _,ip := range server.ips {
			data := sshExec(server.addr,fmt.Sprintf("docker exec whiteblock-node%d cleos --wallet-url http://%s:8899 create key | awk '{print $3}'",node,ip))
